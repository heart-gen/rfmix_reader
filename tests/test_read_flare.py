...
21\t5030578\trs1\tC\tT\t.\tPASS\t.\tGT:AN1:AN2\t0|0:0:0\t0|1:0:1
21\t5030588\trs2\tT\tC\t.\tPASS\t.\tGT:AN1:AN2\t0|0:1:0\t0|0:0:0
"""
...
def test_load_vcf_info(tmp_flare_dir):
    ...
    assert "chromosome" in chunks[0].columns
    assert "physical_position" in chunks[0].columns
...
def test_read_anc(tmp_flare_dir):
    ...
    assert set(df.columns) >= {"SAMPLE", "EUR", "AFR"}
...
def test_read_flare(tmp_flare_dir, monkeypatch):
    ...
    assert isinstance(loci_df, pd.DataFrame)
    assert isinstance(g_anc, pd.DataFrame)

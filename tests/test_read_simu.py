...
def realistic_vcf(tmp_path):
    ...
    vcf_path = tmp_path / "chr21.vcf.gz"
...
def test_read_simu_runs(realistic_vcf):
    ...
    # loci_df should have one variant
    assert loci_df.shape[0] == 1
    # global ancestry has 5 samples
    assert g_anc.shape[0] == 5
    assert set(["CEU", "YRI"]).issubset(set(g_anc.columns))
    # local_array shape: (variants, samples, ancestries)
...
def test_read_simu_stress(tmp_path):
    ...
    # Alternate YRI/CEU across samples
    pops = []
    ...
    # One variant, 50 samples, 2 ancestries
    assert loci_df.shape[0] == 1
    assert g_anc.shape[0] == n_samples
    assert local_array.shape == (1, n_samples, 2)
    # Check that ancestry proportions are valid per sample (row sums ~1)

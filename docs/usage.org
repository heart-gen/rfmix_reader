...
- =interpolation=: ="linear"= (default), ="nearest"=, ="stepwise"=, or
  ...
  built with ~split_to_haplotypes~, and must be explicitly enabled with
  =allow_hmm=True=.
...
falls back to NumPy. Non-HMM methods operate on diploid-summed trajectories and
preserve the original ancestry dimension.
